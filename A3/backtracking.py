...
class UnassignedVars:
    ...
    def __init__(self, select_criteria, csp):
        if select_criteria not in ['random', 'fixed', 'mrv']:
            print "Error UnassignedVars given an illegal selection criteria {}. " \
...
def bt_search(algo, csp, variableHeuristic, allSolutions, trace):
    ...
    if variableHeuristic not in varHeuristics:
        print "Error. Unknown variable heuristics {}. Must be one of {}.".format(
            variableHeuristic, varHeuristics)
    if algo not in algorithms:
        print "Error. Unknown algorithm heuristics {}. Must be one of {}.".format(
            algo, algorithms)
...
def FC(unAssignedVars, csp, allSolutions, trace):
    """Forward checking search.
       ...
       allSolutions is True if you want all solutions.
       trace if you want some tracing of variable assignments tried and constraints failed.
    ...
    if unAssignedVars.empty():
        return [[(var, var.getValue()) for var in csp.variables()]]
