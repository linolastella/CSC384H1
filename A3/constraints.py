...
class TableConstraint(Constraint):
    ...
    def hasSupport(self, var, val):
        ...
        for assignment in self.satAssignments:
            ...
            if found:  # if found still true the assignment worked. We can stop
                break
...
class QueensTableConstraint(TableConstraint):
    ...
    def __init__(self, name, qi, qj, i, j):
        ...
        scope = [qi, qj]
        satisfyingAssignments = [[col_i + 1, col_j + 1] for col_i in range(n) for col_j in range(n)
...
class NValuesConstraint(Constraint):
    """NValues constraint over a set of variables.
       ...
       NValuesConstraint('test_nvalues', [V1, V2, V3, V4], [3,2], 2, 3)
       will only be satisfied by assignments such that at least 2
    ...
    def check(self):
        ...
        for var in self.scope():
            ...
            if var.getValue() in self._required:
                assignedToRequired += 1

        return self._lb <= assignedToRequired <= self._ub
    ...
    def hasSupport(self, var, val):
        ...
        def valsInRange(lst):
            """tests a list of assignments which are pairs (var, val)
               to see if they can satisfy the NValues constraint
            """
            vals = [val for (var, val) in lst]
            num_in_required = sum(map(lambda val: self._required.count(val), vals))

            return self._lb <= num_in_required + self.arity() - len(vals) and num_in_required <= self._ub

        varsToAssign = self.scope()
        varsToAssign.remove(var)
        x = findvals(varsToAssign, [(var, val)], valsInRange, valsInRange)
        return x
