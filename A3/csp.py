...
class Variable:
    ...
    def setValue(self, value):
        if value is not None and value not in self._dom:
    ...
    def pruneValue(self, value, reasonVar, reasonVal):
        ...
        if dkey not in Variable.undoDict:
    ...
    def __init__(self, name, variables, constraints):
        ...
        for v in varsInCnst:
            if v not in variables:
                print "Error: variable {} appears in constraint but specified as one of the variables of the CSP {}".\
                    format(v.name(), self.name())
