...
# test maintenance frequency---24 solutions all with 'AC03' in the middle
...
# test maintenance frequency---zero solutions
...
def check_plane_solution(pp, s):
    """Check is solution is a valid solution to plane problem pp.
       ...
       flies in the sequence flown. E.g., if solution is
       [['AC-1', 'AC01', 'AC02'], ['AC-2', 'AC03', 'AC04'], ['AC-3']]
       then there are three planes AC-1', 'AC-2', and 'AC-3'. AC-1 first
       flies flight AC01 then AC02. AC-2 first flies AC03 then AC04, and
    ...
    for i, pl in enumerate(s):
        ...
        for j in range(len(flights) - 1):
            if (flights[j], flights[j + 1]) not in pp.can_follow:
                print "Error solution invalid, plane {} scheduled to fly invalid flight sequence ({},{})".format(plane,
                                                                                                                 flights
                                                                                                                 [j],
                                                                                                                 flights
                                                                                                                 [j + 1]
        ...
        for j in range(len(flights) - pp.min_maintenance_frequency + 1):
            ...
            for k in range(pp.min_maintenance_frequency):
                if flights[j + k] in pp.maintenance_flights:
                    m_count += 1
            if m_count == 0:
                print "Error solution invalid, plane {} flys to many flights before maintenance {}".format(plane,
                                                                                                           flights)
                return False
    return True
...
if __name__ == '__main__':
    ...
    parser.add_argument("-a", "--algorithm", help="which backtracking algorithm to use", choices=['BT', 'FC', 'GAC'],
                        default='GAC')
    parser.add_argument("-c", "--allSolns", help="Complete search (Find all solutions)", action="store_true")
    parser.add_argument("-v", "--varHeur", help="Heuristic for selecting next variable to assign",
                        choices=['fixed', 'random', 'mrv'], default='mrv')
