...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        Pr(noisyDistance | Xt=p) =
            Pr(noisyDistance | manhattanDistance(p,pacmanPosition))
        ...
        * When a ghost is captured by Pacman, all beliefs should be
        ...
        for p in self.legalPositions:
            ...
            if emissionModel[trueDistance] > 0 and noisyDistance is not None:
    ...
    def elapseTime(self, gameState):
        ...
        HINT. obtaining newPosDist is relatively expensive.  If you
              look carefully at the HMM "progress in time" equation
              you will see that you can organize the computation so
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        div = self.numParticles / len(self.legalPositions)
        rem = self.numParticles % len(self.legalPositions)
        req = div if rem == 0 else div + 1

        self.particles = random.sample(self.legalPositions * req, self.numParticles)
        "*** END YOUR CODE HERE ***"
    ...
    def observe(self, observation, gameState):
        ...
        re-weighting based on observation. If this happens, re-sample particles
        ...
        if noisyDistance is None:
            for i in range(self.numParticles):
                updated.append(self.getJailPosition())
            self.particles = updated
        else:
            particleToWeights = util.Counter()
            for particle in self.particles:
                dist = util.manhattanDistance(particle, pacmanPosition)
                particleToWeights[particle] += emissionModel[dist]

            if particleToWeights.totalCount() > 0:
                for i in range(self.numParticles):
                    updated.append(util.sample(particleToWeights))
                self.particles = updated
            else:
                self.initializeUniformly(gameState)
        "*** END YOUR CODE HERE ***"
    ...
    def elapseTime(self, gameState):
        ...
        newParticles = []
        for particle, count in allPossible.items():
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        self.particles = [pos for pos in allPermutations][-self.numParticles:]
        "*** END YOUR CODE HERE ***"
    ...
    def observeState(self, gameState):
        ...
        2) Otherwise after all new particles have been generated by
           re-sampling you must check if any ghosts have been captured
           by pacman (noisyDistances[i] will be None if ghost i has
           ben captured).
        ...
        for particle in self.particles:
            ...
            for ghost in range(self.numGhosts):
                if noisyDistances[ghost] is not None:
                    ...
                    weight *= emissionModels[ghost][dist]
            allWeights[particle] += weight

        if allWeights.totalCount() > 0:
            self.particles = [util.sample(allWeights) for i in range(self.numParticles)]
        else:
            self.initializeParticles()
        "*** END YOUR CODE HERE ***"
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            "*** YOUR CODE HERE ***"
            for i in range(self.numGhosts):
                newPosDist = getPositionDistributionForGhost(setGhostPositions(gameState, oldParticle), i,
                                                             self.ghostAgents[i])
